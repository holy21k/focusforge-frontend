    ...
    <div style={{ backgroundColor: bg, minHeight: '100vh', fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", color: tp, overflowX: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .social-a { width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;transition:transform .2s;text-decoration:none; }
        ...
        @media(max-width:768px){
          ...
          .step-g { grid-template-columns:1fr 1fr !important; }
      ...
      {/* ══ HERO ═════════════════════════════════════════════════════════ */}
      <section style={{ padding:'120px 40px 72px', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', inset:0, pointerEvents:'none', overflow:'hidden' }}>
          <div style={{ position:'absolute', width:600, height:600, borderRadius:'50%', background:'radial-gradient(circle,#8b5cf6,transparent 70%)', opacity:0.07, top:-100, left:-100, filter:'blur(60px)' }} />
          <div style={{ position:'absolute', width:400, height:400, borderRadius:'50%', background:'radial-gradient(circle,#ef4444,transparent 70%)', opacity:0.05, bottom:0, right:200, filter:'blur(60px)' }} />
        </div>

        <div style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:56, alignItems:'center', position:'relative' }} className="hero-g">
          {/* Left */}
          <div>
            ...
            <h1 style={{ fontWeight:700, fontSize:'2.75rem', lineHeight:1.15, letterSpacing:'-0.02em', color:tp, marginBottom:20, fontFamily:"'DM Sans', sans-serif" }}>
      ...
      <div style={{ borderTop:`0.5px solid ${border}`, borderBottom:`0.5px solid ${border}` }}>
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'48px 40px', display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:32, textAlign:'center' }} className="step-g">
          {[{ n:'87%', l:'Prediction accuracy' }, { n:'12+', l:'Coach personalities' }, { n:'5', l:'AI insights daily' }, { n:'0', l:'Empty encouragements' }].map(({ n, l }) => (
            <div key={l}>
              <div className="syne" style={{ fontSize:'2.5rem', fontWeight:800, background:'linear-gradient(135deg,#8b5cf6,#a78bfa)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', lineHeight:1, marginBottom:6 }}>{n}</div>
      ...
      <section id="features" style={{ padding:'100px 40px' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:64 }}>
            ...
            <h2 className="syne" style={{ fontWeight:800, fontSize:'2.4rem', letterSpacing:'-0.02em', color:tp, marginBottom:14 }}>Everything you need to stay productive</h2>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:20 }} className="feat-g">
            {features.map((f, i) => (
      ...
      {/* ══ HOW IT WORKS ═════════════════════════════════════════════════ */}
      <section id="how-it-works" style={{ padding:'100px 40px', backgroundColor:altBg }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:64 }}>
            <h2 className="syne" style={{ fontWeight:800, fontSize:'2.4rem', letterSpacing:'-0.02em', color:tp, marginBottom:14 }}>How FocusForge works</h2>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:20, position:'relative' }} className="step-g">
            {steps.map((s, i) => (
      ...
      <section id="faq" style={{ padding:'100px 40px' }}>
        <div style={{ maxWidth:720, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:56 }}>
            <h2 className="syne" style={{ fontWeight:800, fontSize:'2.4rem', letterSpacing:'-0.02em', color:tp, marginBottom:14 }}>Frequently Asked Questions</h2>
      ...
      {/* ══ ABOUT ════════════════════════════════════════════════════════ */}
      <section style={{ padding:'100px 40px', backgroundColor:altBg }}>
        <div style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:80, alignItems:'center' }} className="about-g">
          <div>
            <h2 className="syne" style={{ fontWeight:800, fontSize:'2.4rem', letterSpacing:'-0.02em', color:tp, marginBottom:20 }}>About FocusForge</h2>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center' }}>
            <img src={MikoLogo} alt="FocusForge illustration" style={{ width:'100%', maxWidth:420, height:'auto' }} />
          </div>
        </div>
      </section>
      ...
      <section style={{ padding:'100px 40px' }}>
        <div style={{ maxWidth:900, margin:'0 auto' }}>
          <div style={{ borderRadius:28, padding:'72px 48px', textAlign:'center', position:'relative', overflow:'hidden', background:'linear-gradient(135deg,#8b5cf6 0%,#6366f1 50%,#4f46e5 100%)' }}>
            ...
            <div style={{ position:'relative', zIndex:1 }}>
              <h2 className="syne" style={{ fontWeight:800, fontSize:'2.4rem', letterSpacing:'-0.02em', color:'#fff', marginBottom:16, lineHeight:1.1 }}>Ready to transform your productivity?</h2>
              ...
              <button
                ...
                style={{ background:'#fff', color:'#7c3aed', border:'none', borderRadius:12, padding:'15px 40px', fontWeight:700, fontSize:'0.95rem', cursor:'pointer', fontFamily:'inherit', display:'inline-flex', alignItems:'center', gap:8, transition:'transform .2s,box-shadow .2s', boxShadow:'0 4px 20px rgba(0,0,0,0.2)' }}
                onMouseOver={e => { e.currentTarget.style.transform='translateY(-2px)'; e.currentTarget.style.boxShadow='0 8px 28px rgba(0,0,0,0.25)'; }}
                onMouseOut={e => { e.currentTarget.style.transform='none'; e.currentTarget.style.boxShadow='0 4px 20px rgba(0,0,0,0.2)'; }}
              >
      ...
      {/* ══ FOOTER ═══════════════════════════════════════════════════════ */}
      <footer style={{ borderTop:`0.5px solid ${border}`, backgroundColor: isDark ? '#000' : '#fff', padding:'64px 40px 40px' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr', gap:48, marginBottom:48 }} className="foot-g">
            <div>
          ...
          <div style={{ paddingTop:24, borderTop:`0.5px solid ${border}`, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
